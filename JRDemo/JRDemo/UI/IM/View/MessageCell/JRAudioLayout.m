...
- (void)configWithMessage:(JRMessageObject *)message shouldShowTime:(BOOL)showTime shouldShowName:(BOOL)showName {
    ...
    _durationLabelAligment = self.message.direction == JRMessageItemDirectionSend ? NSTextAlignmentLeft:NSTextAlignmentRight;
    _durationLabelTextColor = self.message.direction == JRMessageItemDirectionSend ? [UIColor whiteColor]:[JRSettings skinColor];
    ...
    NSString *imageSepatorName = self.message.direction == JRMessageItemDirectionSend ? @"Sender" : @"Receiver";
    UIColor *imageColor = self.message.direction == JRMessageItemDirectionSend ? [UIColor whiteColor] : [JRSettings skinColor];
